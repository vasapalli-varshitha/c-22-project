...
function setup() {
  ...
  ball_1= Bodies.circle(300,200,35,ball_options);
  World.add(world,ball_1);

  ball_2= Bodies.circle(350,200,35,ball_options);
  World.add(world,ball_2);

  ball_3= Bodies.circle(400,200,35,ball_options);
  World.add(world,ball_3);

  ball_4= Bodies.circle(450,200,35,ball_options);
  World.add(world,ball_4);

  ball_5= Bodies.circle(500,200,35,ball_options);
...
function draw() {
  ...
  ellipse(ball_1.position.x,ball_1.position.y,35);
  ellipse(ball_2.position.x,ball_2.position.y,35);
  ellipse(ball_3.position.x,ball_3.position.y,35);
  ellipse(ball_4.position.x,ball_4.position.y,35);
  ellipse(ball_5.position.x,ball_5.position.y,35);
...
function keyPressed()
{
  if(keyCode===RIGHT_ARROW)
    {
      Matter.Body.applyForce(ball_1,{x:0,y:0},{x:0.10,y:0});
    }

    if(keyCode===DOWN_ARROW)
    {
      Matter.Body.applyForce(ball_2,{x:0,y:0},{x:0.10,y:0});
    }
	

  if(keyCode===LEFT_ARROW)
    {
      Matter.Body.applyForce(ball_3,{x:0,y:0},{x:0.10,y:0});
    }

    if(keyCode===UP_ARROW)
    {
      Matter.Body.applyForce(ball_4,{x:0,y:0},{x:0.10,y:0});
    }

	if(keyCode==="5")
    {
      Matter.Body.applyForce(ball_5,{x:0,y:0},{x:0.10,y:0});
    }
